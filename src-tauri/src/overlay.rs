...
/// Shows the improving overlay window
pub fn show_improving_overlay(app_handle: &AppHandle) {
    // Check if overlay should be shown based on position setting
    let settings = settings::get_settings(app_handle);
    if settings.overlay_position == OverlayPosition::None {
        return;
    ...
    if let Some(overlay_window) = app_handle.get_webview_window("recording_overlay") {
        ...
        // Emit event to switch to improving state
        let _ = overlay_window.emit("show-overlay", "improving");
    }
}

/// Shows the generating overlay window
pub fn show_generating_overlay(app_handle: &AppHandle) {
    ...
    if let Some(overlay_window) = app_handle.get_webview_window("recording_overlay") {
        ...
        // Emit event to switch to generating state
        let _ = overlay_window.emit("show-overlay", "generating");
    }
}
